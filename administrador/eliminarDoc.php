        ...
        if (isset($_POST['btnEliminar']) ) {        
            ...
            if($isValidData){
                ...
                $dbConnection = DBConnection();
                    ...
                    {
                        DeleteDocente($dbConnection, $Docente);
                        LimpiaEntrada();
                       echo '<h3>Matricula Eliminada con exito.</h3>';
                        }
                    else {
...
<br>
        ...
        <br>
        <label for="">Seleccione el docente que desea eliminar:</label>
        <select name="id_docente" id="">
            <?php

            $con = conexion();

            $sql = 'SELECT id_alumno_x_curso, nombre1_doce , apellido1_doce  FROM docentes';
            $query = mysqli_query($con,$sql);

            while($row=mysqli_fetch_array($query)){
                $idDoc = $row['id_docente'];
                $Nombre = $row['nombre1_doce'];
                $Apellido = $row['apellido1_doce'];
                ?>
                    <option value="<?php echo $idDoc ?>"><?php echo $Nombre.' '. $Apellido ?></option>
                <?php       
                }
            ?>      
            <br>
